import { pgTable, text, timestamp, index, foreignKey, jsonb, uuid, unique, integer, bigserial, pgView } from "drizzle-orm/pg-core"
...
export const qrConfigs = pgTable("qr_configs", {
	catalogue: text().default(').notNull(),
...
export const analytics = pgTable("analytics", {
	...
	index("analytics_user_id_date_idx").using("btree", table.userId.asc().nullsLast().op("timestamptz_ops"), table.date.desc().nullsFirst().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
